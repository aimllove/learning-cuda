        ...
        bool useDevice=false, dim3 blockSize=dim3(1), int nStreams=1)
{
	...
	else // Use device
	{
        ...
        // VuMN: size of each stream
        int streamSize = n / nStreams;
        // VuMN: remainder, to compensate to the last stream
        int remain = n % nStreams;
        // VuMN: number of bytes to to copy data (both to and from) for each stream
        size_t streamBytes = streamSize * sizeof(int);
        for (int i = 0; i < nStreams; ++i)
        {
            // VuMN: offset, the position to start copy data (both to and from) for each stream
            int offset = streamSize * i;
            // VuMN: when it comes to the last stream, do compensate
            if (i == nStreams - 1 && nStreams != 1)
            {
                streamBytes += remain * sizeof(int);
                streamSize += remain;
            }
            // VuMN: H2D
