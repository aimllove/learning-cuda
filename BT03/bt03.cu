...
{
	// TODO
	// Get the current (output) point (row, col) index
	int iC = threadIdx.x + blockIdx.x * blockDim.x;
	int iR = threadIdx.y + blockIdx.y * blockDim.y;
	...
	if (iC < width && iR < height)
	{
		...
		for (int rFilter = -centerIdx; rFilter < centerIdx + 1; rFilter++)
		{
			for (int cFilter = -centerIdx; cFilter < centerIdx + 1; cFilter++)
			{
				...
				int rPatched = iR + rFilter, cPatched = iC + cFilter;
				int rConv = min(max(rPatched, 0), height - 1);
				int cConv = min(max(cPatched, 0), width - 1);
		...
		int i = iR * width + iC;	
		outPixels[i] = make_uchar3(sumX, sumY, sumZ);
	}
}
        ...
        uchar3 * outPixels)
{
    ...
    // Current (output) point (row, col) index
	int iC = tx + blockIdx.x * blockDim.x;
	int iR = ty + blockIdx.y * blockDim.y;
    ...
    if (iC < width && iR < height)
	{
        // Thread copies its own data to shared memory
        s_inPixels[(ty + centerIdx) * s_width + (tx + centerIdx)] = inPixels[iR * width + iC];

        // Diagonally copying
        int inPixelsR, inPixelsC;
        // Top-left
        if (tx < filterWidth - 1 && ty < centerIdx ||
            tx < centerIdx && ty < filterWidth - 1)
        {
            inPixelsR = max(0, iR - centerIdx);
            inPixelsC = max(0, iC - centerIdx);
            s_inPixels[ty * s_width + tx] = inPixels[inPixelsR * width + inPixelsC];
        }

        // Top-right
        if (ty < centerIdx ||
            tx >= blockDim.x - centerIdx && ty < blockDim.y - centerIdx - 1)
        {
            inPixelsR = max(0, iR - centerIdx);
            inPixelsC = min(iC + centerIdx, width - 1);
            s_inPixels[ty * s_width + tx + filterWidth - 1] = inPixels[inPixelsR * width + inPixelsC];
        }

        // Bottom-left
        if (tx < centerIdx ||
            ty >= blockDim.y - centerIdx && tx < blockDim.x - centerIdx - 1)
        {
            inPixelsR = min(iR + centerIdx, height - 1);
            inPixelsC = max(0, iC - centerIdx);
            s_inPixels[(ty + filterWidth - 1) * s_width + tx] = inPixels[inPixelsR * width + inPixelsC];
        }

        // Bottom-right
        if (tx >= blockDim.x - centerIdx ||
            ty >= blockDim.y - centerIdx)
        {
            inPixelsR = min(iR + centerIdx, height - 1);
            inPixelsC = min(iC + centerIdx, width - 1);
            s_inPixels[(ty + filterWidth - 1) * s_width + tx + filterWidth - 1] = inPixels[inPixelsR * width + inPixelsC];
        }
        __syncthreads();
		...
		for (int rFilter = -centerIdx; rFilter < centerIdx + 1; rFilter++)
		{
			...
			{
				int iFilter = (rFilter + centerIdx) * filterWidth + (cFilter + centerIdx);
                int rConv = ty + centerIdx + rFilter;
                int cConv = tx + centerIdx + cFilter;
				// The image element to be multiplied at this step
                int iConv = rConv * s_width + cConv;
				// A single multiplication
				sumX += s_inPixels[iConv].x * filter[iFilter];
				sumY += s_inPixels[iConv].y * filter[iFilter];
				sumZ += s_inPixels[iConv].z * filter[iFilter];
			}
		}
		// The actual output element
		int i = iR * width + iC;
    ...
    if (iC < width && iR < height)
	{
		...
		for (int rFilter = -centerIdx; rFilter < centerIdx + 1; rFilter++)
		{
			for (int cFilter = -centerIdx; cFilter < centerIdx + 1; cFilter++)
			{
                ...
                int iConv = rConv * s_width + cConv;
				// A single multiplication
				sumX += s_inPixels[iConv].x * dc_filter[iFilter];
				sumY += s_inPixels[iConv].y * dc_filter[iFilter];
				sumZ += s_inPixels[iConv].z * dc_filter[iFilter];
			}
		}
		// The actual output element
		int i = iR * width + iC;
        ...
        bool useDevice=false, dim3 blockSize=dim3(1, 1), int kernelType=1)
{
	...
	else // Use device
	{
		...
		else if (kernelType == 2)
		{
            ...
            int s_width = blockSize.x + filterWidth - 1;
            int s_height = blockSize.y + filterWidth - 1;
            blurImgKernel2<<<gridSize, blockSize, s_width * s_height * sizeof(uchar3)>>>(d_inPixels, width, height, d_filter, filterWidth, d_outPixels);
		}
		else
		{
            ...
            int s_width = blockSize.x + filterWidth - 1;
            int s_height = blockSize.y + filterWidth - 1;
            blurImgKernel3<<<gridSize, blockSize, s_width * s_height * sizeof(uchar3)>>>(d_inPixels, width, height, filterWidth, d_outPixels);
		}
		timer.Stop();
