		...
		bool useDevice=false, dim3 blockSize=dim3(1))
{
	...
	else // use device
	{
		...
		CHECK(cudaMalloc(&d_inPixels, 3 * width * height * sizeof(uint8_t)));
		CHECK(cudaMalloc(&d_outPixels, width * height * sizeof(uint8_t)));

		// TODO: Copy data to device memories
		CHECK(cudaMemcpy(d_inPixels, inPixels, 3 * width * height * sizeof(uint8_t), cudaMemcpyHostToDevice));
		...
		{
			printf("Error: kernel did not execute properly\n");
